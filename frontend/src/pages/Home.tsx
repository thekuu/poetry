...
export default function Home() {
    ...
    return (
        <div className="space-y-12 sm:space-y-16 w-full max-w-full min-w-0 overflow-x-hidden">
            {!category && type === 'formal' && (
                <section className="text-center py-16 sm:py-24 px-2 sm:px-4 bg-[#FAF8F5] relative border-b border-[#EAE5D9] w-full max-w-full min-w-0 overflow-hidden">
                    <Feather className="w-8 h-8 mx-auto text-[#8B7355] mb-6 sm:mb-8 stroke-[1.5]" />
                    <h1 className={`text-2xl sm:text-5xl text-[#2C2C2C] mb-6 sm:mb-8 tracking-wide break-words max-w-full ${language === 'am' ? 'font-amharic font-medium' : 'font-serif'}`}>{t('formalPoemsTitle')}</h1>
                    <div className="w-16 h-px bg-[#D9D1C7] mx-auto mb-6 sm:mb-8"></div>
                    <p className="text-[#8B8476] text-base sm:text-xl max-w-lg mx-auto font-serif italic leading-relaxed px-2">
            ...
            {!category && type === 'prompt' && (
                <section className="text-center py-16 sm:py-24 px-2 sm:px-4 bg-[#FAF8F5] relative border-b border-[#EAE5D9] w-full max-w-full min-w-0 overflow-hidden">
                    ...
                    <h1 className={`text-2xl sm:text-5xl text-[#2C2C2C] mb-6 sm:mb-8 tracking-wide break-words max-w-full ${language === 'am' ? 'font-amharic font-medium' : 'font-serif'}`}>{t('conversationsTitle')}</h1>
                    <div className="w-16 h-px bg-[#D9D1C7] mx-auto mb-6 sm:mb-8"></div>
                    <p className="text-[#8B8476] text-base sm:text-xl max-w-lg mx-auto font-serif italic leading-relaxed px-2">
            ...
            <section className="w-full max-w-full min-w-0 overflow-hidden">
                <div className="flex items-center justify-center mb-10 sm:mb-16">
                    ...
                    <h2 className="text-base sm:text-xl font-serif text-[#8B8476] tracking-wider uppercase sm:px-8 text-center break-words max-w-full">{pageTitle}</h2>
                    <div className="h-px bg-[#EAE5D9] flex-1 hidden sm:block"></div>
                </div>
                ...
                {poems?.length === 0 ? (
                    ...
                    <div className="flex flex-col gap-8 sm:gap-12 w-full max-w-full min-w-0">
                        {poems?.map((poem) => (
                            <Link key={poem.id} to={`/poems/${poem.id}`} className="group block bg-white p-4 sm:p-14 rounded-2xl shadow-sm border border-[#F3F0EA] hover:shadow-md hover:border-[#EAE5D9] transition-all duration-500 relative w-full max-w-full min-w-0 overflow-hidden box-border">
                                <div className="flex flex-col items-center text-center">
                                    <h3 className="text-xl sm:text-3xl font-serif text-[#2C2C2C] mb-4 sm:mb-6 transition-colors break-words max-w-full">{poem.title}</h3>
                                    <div className="w-12 h-px bg-[#EAE5D9] mb-6 sm:mb-10 group-hover:bg-[#8B7355] transition-colors duration-500"></div>
                                </div>
                                <div className="w-full max-w-full mx-auto min-w-0 overflow-hidden">
                                    <div 
                                        className="poem-content text-[#5C564D] line-clamp-4 whitespace-pre-wrap leading-[2.2] mb-8 sm:mb-12 text-base sm:text-xl text-left w-full max-w-full break-words break-all min-w-0 overflow-hidden"
                                        dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(formatPoemContent(poem.content)) }}
                                    />
                                </div>
                                <div className="flex flex-col sm:flex-row items-center justify-between text-sm text-[#A39D93] pt-6 sm:pt-8 border-t border-[#F3F0EA] gap-3 sm:gap-4 w-full min-w-0">
                                    <span className="font-serif italic text-[#8B8476] text-sm sm:text-lg break-words text-center sm:text-left min-w-0 max-w-full">— {poem.authorName || t('unknownAuthorWithParen')}</span>
